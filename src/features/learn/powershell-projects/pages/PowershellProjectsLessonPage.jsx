...
import PowershellProjectsCodeChallenge from "../components/PowershellProjectsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "powershellprojects";

export default function PowershellProjectsLessonPage() {
  ...
  const lesson = POWERSHELLPROJECTS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = POWERSHELLPROJECTS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = POWERSHELLPROJECTS_LESSONS[lessonIdx - 1];
  const next = POWERSHELLPROJECTS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "PowerShell Projects",
    language: "PowerShell",
    lesson,
  ...
  const earnedXP = POWERSHELLPROJECTS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PowerShell Projects"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`powershellprojects:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PowerShell Projects`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <PowershellProjectsCodeChallenge
