...
import {
  POWERSHELLADMINISTRATION_CHAPTERS,
  POWERSHELLADMINISTRATION_LESSONS,
  POWERSHELLADMINISTRATION_TOTAL_XP,
} from "../data/powershellAdministrationCurriculum";
import usePowershellAdministrationProgress from "../hooks/usePowershellAdministrationProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/powershell-administration";
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["ch-1"],
...
export default function PowershellAdministrationHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = usePowershellAdministrationProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = POWERSHELLADMINISTRATION_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / POWERSHELLADMINISTRATION_LESSONS.length) * 100) || 0;
  const nextLesson =
    POWERSHELLADMINISTRATION_LESSONS.find((lesson) => !progress[lesson.id]) ||
    POWERSHELLADMINISTRATION_LESSONS[0];
  const resumeLesson =
    POWERSHELLADMINISTRATION_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = POWERSHELLADMINISTRATION_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => POWERSHELLADMINISTRATION_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return POWERSHELLADMINISTRATION_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub powershell-administration-hub">
      <div className="oops-hero powershell-administration-hero">
        <Link
        ...
        <p className="oops-hero-sub">
          Master PowerShell Administration with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${POWERSHELLADMINISTRATION_LESSONS.length} lessons · ${earnedXP}/${POWERSHELLADMINISTRATION_TOTAL_XP} XP`
                  : `Sign in to track progress · ${POWERSHELLADMINISTRATION_LESSONS.length} lessons`}
              </span>
          ...
          {!isAuthenticated && (
            <div className="oops-auth-gate oops-auth-gate-hub">
              <p>
                Create a free account to run POWERSHELL challenges, earn XP, and save
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{POWERSHELLADMINISTRATION_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{POWERSHELLADMINISTRATION_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{POWERSHELLADMINISTRATION_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {POWERSHELLADMINISTRATION_CHAPTERS.length} chapters · {" "}
      {POWERSHELLADMINISTRATION_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = POWERSHELLADMINISTRATION_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={POWERSHELLADMINISTRATION_CHAPTERS}
        progress={progress}
      ...
      />
      <CourseCertificate
        courseName="PowerShell Administration"
        totalLessons={POWERSHELLADMINISTRATION_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={POWERSHELLADMINISTRATION_TOTAL_XP}
      />
    </div>
  );
}
