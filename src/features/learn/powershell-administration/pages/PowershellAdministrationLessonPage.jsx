...
import PowershellAdministrationCodeChallenge from "../components/PowershellAdministrationCodeChallenge";
import {
...
const READ_GATE_PREFIX = "powershelladministration";

export default function PowershellAdministrationLessonPage() {
  ...
  const lesson = POWERSHELLADMINISTRATION_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = POWERSHELLADMINISTRATION_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = POWERSHELLADMINISTRATION_LESSONS[lessonIdx - 1];
  const next = POWERSHELLADMINISTRATION_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "PowerShell Administration",
    language: "PowerShell",
    lesson,
    ...
    return (
      <div className="oops-not-found">
        ...
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to PowerShell Administration
        </button>
      </div>
    );
  }
  ...
  const earnedXP = POWERSHELLADMINISTRATION_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PowerShell Administration"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`powershelladministration:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PowerShell Administration`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <PowershellAdministrationCodeChallenge
