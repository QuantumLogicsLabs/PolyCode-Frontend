...
const LEARNING_PATH = [
  {
    ...
    color: "#16a34a",
    summary: "Classes, objects, and the initialize constructor.",
  },
  {
    level: "Intermediate",
    chapters: RUBY_OOP_CHAPTERS.filter((ch) => ch.stage === "intermediate").map((ch) => ch.id),
    color: "#0ea5e9",
    summary: "Inheritance, super, and how objects compare themselves.",
  },
  {
    level: "Pro",
    chapters: RUBY_OOP_CHAPTERS.filter((ch) => ch.stage === "pro").map((ch) => ch.id),
    color: "#7e22ce",
    summary: "Class-level state, factory methods, and duck-typed designs.",
  },
  {
    level: "Advanced",
    chapters: RUBY_OOP_CHAPTERS.filter((ch) => ch.stage === "advanced").map((ch) => ch.id),
    color: "#dc2626",
    summary: "Mixins, composition, delegation, and immutable value objects.",
  },
];
...
export default function RubyOopHub() {
  ...
  const pct =
    Math.round((completedCount / RUBY_OOP_LESSONS.length) * 100) || 0;
  const nextLesson =
    RUBY_OOP_LESSONS.find((lesson) => !progress[lesson.id]) ||
    RUBY_OOP_LESSONS[0];
  const resumeLesson =
    RUBY_OOP_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  ...
  const filteredLessons = useMemo(() => {
    ...
    return RUBY_OOP_LESSONS.filter((lesson) => {
      const chapter = RUBY_OOP_CHAPTERS.find((c) => c.id === lesson.chapterId);
    ...
    <div className="oops-hub ruby-oop-hub">
      <div className="oops-hero ruby-oop-hero">
        ...
        <p className="oops-hero-sub">
          Design Ruby objects that behave well — classes and inheritance,
          equality and comparison, class-level design, composition, and
          immutable value objects, with hands-on Ruby challenges.
        </p>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {RUBY_OOP_CHAPTERS.length} chapters · {RUBY_OOP_LESSONS.length} lessons
          </small>
        </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search classes, equality, mixins..."
              aria-label="Search Ruby OOP lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Ruby OOP lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{RUBY_OOP_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{RUBY_OOP_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{RUBY_OOP_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
