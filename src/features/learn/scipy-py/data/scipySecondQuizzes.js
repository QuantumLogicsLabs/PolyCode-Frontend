...
export const SCIPY_SECOND_QUIZZES = {
  ...
  "scipy-16": q(
    "What is the plain difference between `pdf(x)` and `cdf(x)` on a distribution?",
    [
      "pdf describes density at x; cdf describes total chance up to x",
      "pdf sends email; cdf opens spreadsheets",
      "pdf is always larger than 100",
      "cdf only works on text files",
    ],
    0,
    "PDF = how dense chance is at a point; CDF = accumulated probability up to that point.",
  ),
  "scipy-17": q(
    "Class scores have mean 88 but variance is very high. What does that suggest?",
    [
      "Scores are spread out — not everyone clustered near 88",
      "Every student scored exactly 88",
      "The mean is wrong and variance fixes it automatically",
      "Variance only counts how many files you saved",
    ],
    0,
    "High variance means values scatter widely around the mean; low variance means a tight cluster.",
  ),
  "scipy-18": q(
    "A small p-value in a t-test suggests…",
    [
      "The sample would be surprising if the assumed mean were true",
      "The experiment is guaranteed to win a Nobel prize",
      "You should never collect data again",
      "Python must be reinstalled",
    ],
    0,
    "A small p-value means the data look unlikely under the simple assumption — not automatic proof.",
  ),
  "scipy-19": q(
