...
import {
  POWERSHELLSCRIPTING_CHAPTERS,
  POWERSHELLSCRIPTING_LESSONS,
  POWERSHELLSCRIPTING_TOTAL_XP,
} from "../data/powershellScriptingCurriculum";
import usePowershellScriptingProgress from "../hooks/usePowershellScriptingProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/powershell-scripting";
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["ch-1"],
...
export default function PowershellScriptingHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = usePowershellScriptingProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = POWERSHELLSCRIPTING_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / POWERSHELLSCRIPTING_LESSONS.length) * 100) || 0;
  const nextLesson =
    POWERSHELLSCRIPTING_LESSONS.find((lesson) => !progress[lesson.id]) ||
    POWERSHELLSCRIPTING_LESSONS[0];
  const resumeLesson =
    POWERSHELLSCRIPTING_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = POWERSHELLSCRIPTING_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => POWERSHELLSCRIPTING_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return POWERSHELLSCRIPTING_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub powershell-scripting-hub">
      <div className="oops-hero powershell-scripting-hero">
        <Link
        ...
        <p className="oops-hero-sub">
          Master PowerShell Scripting with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${POWERSHELLSCRIPTING_LESSONS.length} lessons · ${earnedXP}/${POWERSHELLSCRIPTING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${POWERSHELLSCRIPTING_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{POWERSHELLSCRIPTING_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{POWERSHELLSCRIPTING_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{POWERSHELLSCRIPTING_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {POWERSHELLSCRIPTING_CHAPTERS.length} chapters · {" "}
      {POWERSHELLSCRIPTING_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = POWERSHELLSCRIPTING_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={POWERSHELLSCRIPTING_CHAPTERS}
        progress={progress}
      ...
      />
      <CourseCertificate
        courseName="PowerShell Scripting"
        totalLessons={POWERSHELLSCRIPTING_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={POWERSHELLSCRIPTING_TOTAL_XP}
      />
    </div>
  );
}
