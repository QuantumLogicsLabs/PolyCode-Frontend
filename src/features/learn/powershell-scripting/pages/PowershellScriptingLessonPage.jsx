...
import PowershellScriptingCodeChallenge from "../components/PowershellScriptingCodeChallenge";
import {
...
const READ_GATE_PREFIX = "powershellscripting";

export default function PowershellScriptingLessonPage() {
  ...
  const lesson = POWERSHELLSCRIPTING_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = POWERSHELLSCRIPTING_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = POWERSHELLSCRIPTING_LESSONS[lessonIdx - 1];
  const next = POWERSHELLSCRIPTING_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "PowerShell Scripting",
    language: "PowerShell",
    lesson,
  ...
  const earnedXP = POWERSHELLSCRIPTING_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PowerShell Scripting"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`powershellscripting:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PowerShell Scripting`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <PowershellScriptingCodeChallenge
