...
const READ_GATE_PREFIX = "web-accessibility";

export default function WebAccessibilityLessonPage() {
  ...
  const lesson = WEB_ACCESSIBILITY_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = WEB_ACCESSIBILITY_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = WEB_ACCESSIBILITY_LESSONS[lessonIdx - 1];
  const next = WEB_ACCESSIBILITY_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Web Accessibility (a11y)",
    language: "HTML/CSS",
    lesson,
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Web Accessibility lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Web Accessibility
        </button>
      </div>
    );
  }
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Web Accessibility (a11y)"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`web-accessibility:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Web Accessibility (a11y)`}
        >
          {tab === "theory" ? (
            <HtmlCssLessonTheory
              lesson={lesson}
