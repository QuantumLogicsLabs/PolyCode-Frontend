...
export default function GoFundamentalsCodeChallenge({
  challenge,
  accentColor,
  isCompleted,
  onComplete,
  ...
  function runTests() {
    ...
    setOutput({
      status: "running",
      stdout: "Running Go compilation checks…",
    });

    window.setTimeout(async () => {
      let expectedOutput = "";
      try {
        const expectedRun = await runGoCode(challenge.solutionCode);
        expectedOutput = formatGoOutput(expectedRun.result);
      } catch {
        expectedOutput = "";
      }
      ...
      let runPayload;
      try {
        runPayload = await runGoCode(code);
      } catch (error) {
        setResults({
          passed: false,
          tests: [
            {
              id: "runtime",
              label: "Go runs without errors",
              passed: false,
              hint: error.message || "Could not execute Go code.",
            },
        ...
        setOutput({
          status: "fail",
          stdout: error.message || "Execution failed",
      ...
      const runtimeError = getGoRuntimeError(runResult);
      const stdout = formatGoOutput(runResult);
          ...
          tests: [
            {
              ...
              label: "Go runs without errors",
              passed: false,
              hint: "Fix the compilation error or panic in Output, then run again.",
            },
            ...challenge.tests.map((test) => ({ ...test, passed: false })),
          ],
        });
      ...
      setOutput({
        ...
        stdout: stdout || "Program exited normally (no console output).",
        expected: expectedOutput,
      });
    ...
    <div className="oops-challenge">
      ...
      <div className="oops-problem-panel">
        ...
        >
          ...
          <pre className="oops-output-body">
            {output?.stdout || "Run your code to see compilation output here."}
          </pre>
        </div>
      </div>
      ...
      <div className="oops-editor-panel">
        <div className="oops-editor-topbar">
          <span className="oops-editor-lang">Go · main.go</span>
          ...
          {results && (
            ...
            >
              {results.passed
                ...
                : `${results.tests.filter((t) => t.passed).length}/${challenge.tests.length} assertions passed`}
            </div>
          )}
          <button
            type="button"
