...
import GoFundamentalsCodeChallenge from "../components/GoFundamentalsCodeChallenge";
import {
...
export default function GoFundamentalsLessonPage() {
  ...
  const lesson = GO_FUNDAMENTALS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = GO_FUNDAMENTALS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = GO_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = GO_FUNDAMENTALS_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = GO_FUNDAMENTALS_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      <OopsSidebar
        ...
        progress={progress}
        chapters={GO_FUNDAMENTALS_CHAPTERS}
        basePath={BASE_PATH}
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Go Fundamentals"
            syncLabel={
              isAuthenticated
                ? "Go progress saved to your account"
        ...
        <LessonContentShell
          storageKey={`go-fundamentals:${lessonId}`}
