...
const READ_GATE_PREFIX = "sass-scss";

export default function SassScssLessonPage() {
  ...
  const lesson = SASS_SCSS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = SASS_SCSS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = SASS_SCSS_LESSONS[lessonIdx - 1];
  const next = SASS_SCSS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Sass & SCSS lesson not found.</p>
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`sass-scss:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Sass & SCSS`}
        >
          {tab === "theory" ? (
            <HtmlCssLessonTheory
              lesson={lesson}
