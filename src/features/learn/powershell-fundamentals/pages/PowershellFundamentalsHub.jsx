...
import {
  POWERSHELLFUNDAMENTALS_CHAPTERS,
  POWERSHELLFUNDAMENTALS_LESSONS,
  POWERSHELLFUNDAMENTALS_TOTAL_XP,
} from "../data/powershellFundamentalsCurriculum";
import usePowershellFundamentalsProgress from "../hooks/usePowershellFundamentalsProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/powershell-fundamentals";
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["ch-1"],
...
export default function PowershellFundamentalsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = usePowershellFundamentalsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = POWERSHELLFUNDAMENTALS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / POWERSHELLFUNDAMENTALS_LESSONS.length) * 100) || 0;
  const nextLesson =
    POWERSHELLFUNDAMENTALS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    POWERSHELLFUNDAMENTALS_LESSONS[0];
  const resumeLesson =
    POWERSHELLFUNDAMENTALS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = POWERSHELLFUNDAMENTALS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => POWERSHELLFUNDAMENTALS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return POWERSHELLFUNDAMENTALS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub powershell-fundamentals-hub">
      <div className="oops-hero powershell-fundamentals-hero">
        <Link
        ...
        <p className="oops-hero-sub">
          Master PowerShell Fundamentals with interactive lessons and practical queries.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${POWERSHELLFUNDAMENTALS_LESSONS.length} lessons · ${earnedXP}/${POWERSHELLFUNDAMENTALS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${POWERSHELLFUNDAMENTALS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{POWERSHELLFUNDAMENTALS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{POWERSHELLFUNDAMENTALS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{POWERSHELLFUNDAMENTALS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
  ...
  <div className="matplotlib-path-label">
    ...
    <small>
      {POWERSHELLFUNDAMENTALS_CHAPTERS.length} chapters · {" "}
      {POWERSHELLFUNDAMENTALS_LESSONS.length} lessons
    </small>
  </div>
  <div className="matplotlib-path-grid">
    {LEARNING_PATH.map((stage) => {
      const stageChapters = POWERSHELLFUNDAMENTALS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={POWERSHELLFUNDAMENTALS_CHAPTERS}
        progress={progress}
      ...
      />
      <CourseCertificate
        courseName="PowerShell Fundamentals"
        totalLessons={POWERSHELLFUNDAMENTALS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={POWERSHELLFUNDAMENTALS_TOTAL_XP}
      />
    </div>
  );
}
