...
import {
  RESPONSIVE_DESIGN_CHAPTERS,
  RESPONSIVE_DESIGN_LESSONS,
  RESPONSIVE_DESIGN_TOTAL_XP,
} from "../data/responsiveDesignCurriculum";
import useResponsiveDesignProgress from "../hooks/useResponsiveDesignProgress";
import useLessonReadGate from "../../shared/useLessonReadGate";
import LessonChallengeTab from "../../shared/LessonChallengeTab";
import { useLessonAssistantContext } from "../../../assistant/hooks/useLessonAssistantContext";

const BASE_PATH = "/learn/responsive-design";
const READ_GATE_PREFIX = "responsive-design";
...
export default function ResponsiveDesignLessonPage() {
  ...
  const {
    ...
    saveCode,
    toggleBookmark,
  } = useResponsiveDesignProgress();
  const codeSaveTimer = useRef(null);

  const lesson = RESPONSIVE_DESIGN_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = RESPONSIVE_DESIGN_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = RESPONSIVE_DESIGN_LESSONS[lessonIdx - 1];
  const next = RESPONSIVE_DESIGN_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Responsive Web Design lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Responsive Web Design
        </button>
      </div>
    );
  }
  ...
  const earnedXP = RESPONSIVE_DESIGN_LESSONS.filter(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      <OopsSidebar
        ...
        progress={progress}
        chapters={RESPONSIVE_DESIGN_CHAPTERS}
        basePath={BASE_PATH}
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Responsive Web Design"
            syncLabel={
              isAuthenticated
            ...
            totalLessons={RESPONSIVE_DESIGN_LESSONS.length}
            earnedXP={earnedXP}
            totalXP={RESPONSIVE_DESIGN_TOTAL_XP}
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`responsive-design:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Responsive Web Design`}
        >
          {tab === "theory" ? (
            <HtmlCssLessonTheory
              lesson={lesson}
