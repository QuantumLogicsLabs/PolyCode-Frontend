...
export default function PyTorchHub() {
  ...
  const bookmarkedLessons = bookmarks
    .map((id) => PYTORCH_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return PYTORCH_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${PYTORCH_LESSONS.length} lessons · ${earnedXP}/${PYTORCH_TOTAL_XP} XP`
                  : `Sign in to track progress · ${PYTORCH_LESSONS.length} lessons`}
              </span>
